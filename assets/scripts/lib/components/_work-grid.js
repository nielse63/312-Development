...
export default class WorkGrid {
	...
	getRandomColor() {
		...
		// const key = 'PuRd'
		let array = [];
		for(let i in colorbrewer[key]) {
			let v = colorbrewer[key][i];
			array.push(v);
		}
		return array.pop();
	}
	...
	colorizeWorkBlocks() {
		...
		this.$workBlocks.each(function() {
			...
			// create gradient
			let darker = tc.darken(35).toHexString();
			let gradient = 'linear-gradient(150deg, ' + darker + ', ' + color + ')';

			// get luma
			let rgb = tc.toRgb();
			let luma = (rgb.r * 0.3) + (rgb.g * 0.59) + (rgb.b * 0.11);
			if( luma > 215 ) {
				$ele.addClass('dark');
			}
